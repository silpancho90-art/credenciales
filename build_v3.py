...
A1 = read_all('archivos 1.xlsx')      # fuente de verdad (datos reales)
SYS = read_all('archivo 2.xlsx')      # sistema base (diseno + consolidado vigente)
...
def fix_name(s):
    """Correccion solicitada: variantes Milenka/Milenko/Milenco -> nombre correcto."""
    s = clean(s)
    u = s.upper()
    if 'ORDO' in u and ('MILEN' in u):
        return 'MILENCA ADRIANA ORDOÑEZ NUÑEZ'
    return s
def norm_sup(s):
    s = fix_name(s); return SUP_MAP.get(s, s)

# ---------------------------------------------------------------------------
# MODELO  (base = archivo 2.xlsx, ya consolidado y editado por el usuario)
# Estructura archivo 2: 3 filas de banner + cabecera (fila idx 3) + datos (idx 4+)
# ---------------------------------------------------------------------------
DATA = 4

personal = []
for row in SYS['PERSONAL'][DATA:]:
    if not clean(row[1] if len(row) > 1 else ''): continue   # Nombre_Completo
    r = (row + [''] * 8)[:8]
    personal.append({'tel': clean(r[0]) or 'POR REGISTRAR', 'nombre': fix_name(r[1]),
                     'rol': clean(r[2]), 'sup': norm_sup(r[3]), 'reg': clean(r[4]),
                     'estado': clean(r[5]), 'cred': clean(r[6]), 'obs': clean(r[7])})
# ids reconstruidos
for i, p in enumerate(personal, 1):
    p['id'] = 'PER-%03d' % i
name2tel = {p['nombre']: p['tel'] for p in personal}

movimientos = []
for row in SYS['MOVIMIENTOS'][DATA:]:
    if not clean(row[0] if row else ''): continue            # ID_Movimiento
    r = (row + [''] * 9)[:9]
    nombre = fix_name(r[5])
    movimientos.append({'n': clean(r[0]), 'fecha': to_int(r[1]), 'tipo': clean(r[2]),
                        'cod': clean(r[3]), 'tel': clean(r[4]) or name2tel.get(nombre, 'POR REGISTRAR'),
                        'nombre': nombre, 'sup': norm_sup(r[6]), 'reg': clean(r[7]), 'obs': clean(r[8])})

inventario = []; seen = set()
for row in SYS['INVENTARIO_GENERAL'][DATA:]:
    cod = clean(row[0] if row else '')
    if not cod.upper().startswith('BEX'): continue
    r = (row + [''] * 12)[:12]
    inventario.append({'cod': cod, 'gen': clean(r[1]) or 'LEGACY (BEX-0xxx)', 'estado': clean(r[2]) or 'DISPONIBLE',
                       'resp': fix_name(r[3]), 'sup': norm_sup(r[5]), 'reg': clean(r[6]),
                       'fecha': to_int(r[7]), 'obs': clean(r[11])})
    seen.add(cod)
# Salvaguarda: si faltara stock BEX-2051..2100, agregarlo (no perder inventario)
...
# supervisores: deducidos de los datos reales, en orden por regional
_sup_seen = []
for rg in regionales:
    for p in personal:
        if p['rol'] == 'SUPERVISOR' and p['reg'] == rg and p['nombre'] not in _sup_seen:
            _sup_seen.append(p['nombre'])
for it in inventario:                       # incluir supervisores presentes en inventario
    if it['sup'] and it['sup'] not in _sup_seen:
        _sup_seen.append(it['sup'])
supervisores = _sup_seen
...
print('OK modelo v3'); print('Supervisores:', len(supervisores)); print('Milenca en sup:', any('MILENCA' in s for s in supervisores))
...
<dxf><font><color rgb="FFC65911"/></font><fill><patternFill><bgColor rgb="FFFCE4D6"/></patternFill></fill></dxf>
</dxfs>
</styleSheet>'''
...
# dxf ids (esquema de colores por estado)
DXF = {'ASIGNADO': 1, 'DISPONIBLE': 0, 'PERDIDO': 2, 'POR ASIGNAR': 3, 'DEVUELTO': 4}
DXF_GREEN, DXF_BLUE, DXF_RED, DXF_YELLOW, DXF_ORANGE = 0, 1, 2, 3, 4
...
def build_personal():
    ...
    sh.cond.append(('F%d:F%d' % (hr + 1, last), [('equal', '"ACTIVO"', DXF_GREEN),
                                                 ('equal', '"INACTIVO"', DXF_RED)]))
    return sh
...
def build_inicio():
    ...
    nav = [
        ...
        ('CONSULTA_RAPIDA', 'Busqueda operativa diaria (un solo cuadro)'),
        ('BUSCADOR', 'Buscar credencial por codigo, nombre o telefono'),
        ('ALERTAS', 'Incidencias: perdidas, sin responsable, duplicados'),
        ('AUDITORIA', 'Trazabilidad de eventos e historial completo'),
...
# ===========================================================================
# HOJAS NUEVAS: CONSULTA_RAPIDA, ALERTAS, AUDITORIA
# ===========================================================================
INVCOLS = ['Codigo_Activo', 'Tipo_Credencial', 'Estado_Actual', 'Responsable_Actual', 'Telefono',
           'Supervisor', 'Regional', 'Fecha_Ultima_Asignacion', 'Fecha_Ultima_Devolucion',
           'Cantidad_Perdidas', 'Dias_En_Uso', 'Observaciones']
PERCOLS = ['Telefono', 'Nombre_Completo', 'Rol', 'Supervisor', 'Regional', 'Estado', 'Credencial_Actual', 'Observaciones']
MOVCOLS = ['ID_Movimiento', 'Fecha', 'Tipo_Movimiento', 'Codigo_Activo', 'Telefono', 'Nombre', 'Supervisor', 'Regional', 'Observaciones']

def build_consulta():
    sh = Sheet('CONSULTA_RAPIDA', tabcolor='27AE60')
    sh.cols = [(1, 1, 3), (2, 2, 26), (3, 3, 44), (4, 4, 18), (5, 5, 16), (6, 6, 16)]
    hr = banner(sh, 'CONSULTA RAPIDA  -  Uso diario operativo', 6,
                'COMO USAR ESTA HOJA',
                'Escriba en la celda amarilla UN solo dato: codigo BEX (ej: BEX-2050), nombre o telefono. '
                'Abajo aparece al instante toda la informacion. No modifique las celdas de resultado.')
    sh.set(5, 2, 'BUSCAR:', 's', S_LBLR); sh.rowheights[5] = 26
    sh.set(5, 3, '', 's', S_INPUT); sh.merges.append('C5:F5')
    K = '$C$5'
    # codigo resuelto (detecta si es BEX, nombre o telefono)
    sh.set(7, 2, 'Codigo detectado:', 's', S_LBLR)
    rc = ('IF(%s="","",IF(LEFT(UPPER(%s),4)="BEX-",%s,IFERROR(XLOOKUP(%s,%s[Nombre_Completo],%s[Credencial_Actual]),'
          'IFERROR(XLOOKUP(%s,%s[Telefono],%s[Credencial_Actual]),""))))' % (K, K, K, K, TPE, TPE, K, TPE, TPE))
    sh.set(7, 3, rc, 'f', S_CEN)
    RC = '$C$7'
    def lk(col): return 'XLOOKUP(%s,%s[Codigo_Activo],%s[%s])' % (RC, TIG, TIG, col)
    sh.set(9, 1, 'RESULTADO', 's', S_SECTION); sh.merges.append('A9:F9')
    fields = [
        ('CODIGO BEX:', 'IF(%s="","(escriba arriba)",IFERROR(%s,"NO ENCONTRADO"))' % (RC, lk('Codigo_Activo')), S_BORD),
        ('ESTADO:', 'IF(%s="","-",LET(e,IFERROR(%s,"NO ENCONTRADO"),IF(e="DISPONIBLE","DISPONIBLE EN OFICINA",IF(e="PERDIDO","PERDIDA",e))))' % (RC, lk('Estado_Actual')), S_BORD),
        ('RESPONSABLE ACTUAL:', 'IF(%s="","-",IFERROR(%s,"-"))' % (RC, lk('Responsable_Actual')), S_BORD),
        ('TELEFONO:', 'IF(%s="","-",IFERROR(%s,"-"))' % (RC, lk('Telefono')), S_BORD),
        ('SUPERVISOR:', 'IF(%s="","-",IFERROR(%s,"-"))' % (RC, lk('Supervisor')), S_BORD),
        ('REGIONAL:', 'IF(%s="","-",IFERROR(%s,"-"))' % (RC, lk('Regional')), S_BORD),
        ('FECHA ASIGNACION:', 'IF(%s="","-",IFERROR(%s,"-"))' % (RC, lk('Fecha_Ultima_Asignacion')), S_DATE),
        ('FECHA DEVOLUCION:', 'IF(%s="","-",IFERROR(%s,"-"))' % (RC, lk('Fecha_Ultima_Devolucion')), S_DATE),
        ('CANTIDAD PERDIDAS:', 'IF(%s="","-",IFERROR(%s,"-"))' % (RC, lk('Cantidad_Perdidas')), S_CEN),
        ('OBSERVACIONES:', 'IF(%s="","-",IFERROR(%s,"-"))' % (RC, lk('Observaciones')), S_BORD),
    ]
    r = 10
    for lbl, f, st in fields:
        sh.set(r, 2, lbl, 's', S_LBLR); sh.set(r, 3, f, 'f', st); sh.merges.append('C%d:F%d' % (r, r)); r += 1
    h = r + 1
    sh.set(h, 1, 'HISTORIAL DE LA CREDENCIAL', 's', S_SECTION); sh.merges.append('A%d:I%d' % (h, h))
    for i, c in enumerate(MOVCOLS, 1):
        sh.set(h + 1, i, c, 's', S_HEAD)
    sh.set(h + 2, 1, 'IFERROR(FILTER(%s,%s[Codigo_Activo]=%s),"Sin historial / escriba un dato valido")' % (TMO, TMO, RC), 'fa', S_BORD)
    return sh

def _report_block(sh, row, title, headers, filter_formula, ncols_merge=14):
    sh.set(row, 1, title, 's', S_SECTION); sh.merges.append('A%d:%s%d' % (row, col_letter(ncols_merge), row))
    for i, c in enumerate(headers, 1):
        sh.set(row + 1, i, c, 's', S_HEAD)
    sh.set(row + 2, 1, filter_formula, 'fa', S_BORD)

def build_alertas():
    sh = Sheet('ALERTAS', tabcolor='C0392B')
    sh.cols = [(i, i, 15) for i in range(1, 13)]
    banner(sh, 'ALERTAS  -  Puntos que requieren atencion', 12,
           'COMO USAR ESTA HOJA',
           'Listas automaticas de incidencias detectadas en tiempo real. Si una lista dice "Ninguna", no hay problemas de ese tipo. '
           'Esta hoja es de solo lectura; corrija los datos en INVENTARIO_GENERAL, PERSONAL o MOVIMIENTOS.')
    _report_block(sh, 5, '1. CREDENCIALES PERDIDAS', INVCOLS,
                  'IFERROR(TAKE(FILTER(%s,%s[Estado_Actual]="PERDIDO"),15),"Ninguna")' % (TIG, TIG))
    _report_block(sh, 23, '2. ASIGNADAS SIN RESPONSABLE DEFINIDO', INVCOLS,
                  'IFERROR(TAKE(FILTER(%s,(%s[Estado_Actual]="ASIGNADO")*((%s[Responsable_Actual]="POR ASIGNAR")+(%s[Responsable_Actual]=""))),15),"Ninguna")' % (TIG, TIG, TIG, TIG))
    _report_block(sh, 41, '3. CREDENCIALES CON ALERTA DE DUPLICADO / CRITICO', INVCOLS,
                  'IFERROR(TAKE(FILTER(%s,ISNUMBER(SEARCH("DUPLICADO",%s[Observaciones]))+ISNUMBER(SEARCH("CRITICO",%s[Observaciones]))),15),"Ninguna")' % (TIG, TIG, TIG))
    _report_block(sh, 59, '4. PERSONAL INACTIVO (verificar credenciales a su nombre)', PERCOLS,
                  'IFERROR(FILTER(%s,%s[Estado]="INACTIVO"),"Ninguno")' % (TPE, TPE), ncols_merge=8)
    return sh

def build_auditoria():
    sh = Sheet('AUDITORIA', tabcolor='8E44AD')
    sh.cols = [(1, 1, 13), (2, 2, 13), (3, 3, 18), (4, 4, 13), (5, 5, 14), (6, 6, 34), (7, 7, 30), (8, 8, 13), (9, 9, 38)]
    banner(sh, 'AUDITORIA  -  Trazabilidad de eventos', 9,
           'COMO USAR ESTA HOJA',
           'Registro de eventos sensibles (cambios de supervisor/regional, reasignaciones, perdidas, devoluciones) '
           'y el historial completo ordenado del mas reciente al mas antiguo. Solo lectura.')
    _report_block(sh, 5, 'EVENTOS DE AUDITORIA (cambios, reasignaciones, perdidas, devoluciones)', MOVCOLS,
                  'IFERROR(TAKE(FILTER(%s,ISNUMBER(MATCH(%s[Tipo_Movimiento],{"CAMBIO SUPERVISOR";"CAMBIO REGIONAL";"REASIGNACION";"PERDIDA";"DEVOLUCION";"REPOSICION"},0))),30),"Sin eventos de auditoria")' % (TMO, TMO), ncols_merge=9)
    _report_block(sh, 40, 'HISTORIAL COMPLETO DE MOVIMIENTOS (mas reciente primero)', MOVCOLS,
                  'IFERROR(SORTBY(FILTER(%s,%s[ID_Movimiento]<>""),%s[Fecha],-1),"Sin movimientos")' % (TMO, TMO, TMO), ncols_merge=9)
    return sh
...
def package(path):
    ...
    sheets = [inicio, dash, consulta, busc, alertas, auditoria, inv, per, mov, par, motor]
